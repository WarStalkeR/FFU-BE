	...
	public class FFU_BE_Prefab_PassiveECMs {
		...
		public static void UpdateCountermeasureModule(ShipModule shipModule, bool initItemData) {
			...
			switch (shipModule.PrefabId) {
				...
				if (!FFU_BE_Defs.survivalTypeIDs[Core.SurvivalType.EWAR].Contains(shipModule.PrefabId)) FFU_BE_Defs.survivalTypeIDs[Core.SurvivalType.EWAR].Add(shipModule.PrefabId);
				...
				shipModule.displayName = Core.TT($"Makeshift <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Electronic countermeasure array that was made from salvage and scrap materials. Although has very questionable quality, still manages somewhat to disrupt hostile targeting systems.");
				...
				shipModule.displayName = Core.TT($"Ancient <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Was manufactured centuries ago and used in every imaginable war and military operation. Heavily wearied down, but still can disrupt hostile targeting systems to certain extent.");
				...
				shipModule.displayName = Core.TT($"Imperial <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"A rare case of technology that was designed and manufactured by Rat Empire from scratch. Has mediocre quality, but still can be used to disrupt hostile targeting systems.");
				...
				shipModule.displayName = Core.TT($"Velocity <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Created by marauders and pirates from various salvaged parts of other ECM arrays, but with professional touch. Has average jamming efficiency of hostile targeting systems.");
				...
				shipModule.displayName = Core.TT($"Reflector <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Manufactured with specialized mirror array that constantly emits ship's reflections that has some chance to fool enemy targeting systems with decent efficiency.");
				...
				shipModule.displayName = Core.TT($"Bionic <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Electronic countermeasure array or organic origin. Grown up in special environment and contains all ports for proper interfacing. Has good jamming efficiency.");
				...
				shipModule.displayName = Core.TT($"Prismatic <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Electronic countermeasure array with built-in prismatic emitters that literally blind hostile targeting systems and sensors. Has very good jamming efficiency.");
				...
				shipModule.displayName = Core.TT($"Repulsor <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"Electronic countermeasure array that uses kinetic energy and unknown principles to disrupt hostile targeting systems and sensors. Has great jamming efficiency.");
				...
				shipModule.displayName = Core.TT($"Quantum <color=#{colorCounter}ff>ECM Array</color>");
				shipModule.description = Core.TT($"This electronic countermeasure array constantly produces quantum chaff with intangibility effect that disrupts hostile targeting systems and sensors with excellent efficiency.");
				...
				if (!FFU_BE_Defs.moduleEmissionPrefabs.ContainsKey(shipModule.PrefabId)) FFU_BE_Defs.moduleEmissionPrefabs.Add(shipModule.PrefabId, -9.0f);
				shipModule.displayName = Core.TT($"ECM Array Artifact");
				...
				shipModule.displayName = $"(ECM) {shipModule.name}";
				break;
			}
